...
//        exit();
    ...
    public function add_robot(){
        ...
        $order=StoreOrderModel::where(['paid'=>1,'pin_type'=>1,'pin_status'=>1])->select();
        if($order){
    ...
    public function is_robot(){
        ...
        $order=StoreOrderModel::where(['add_time'=>['<',$spell_time],'paid'=>1,'pin_type'=>1,'pin_status'=>1])->select();
    ...
    public function return_money(){
        ...
        //加入佣金
        $integral_info=UserBill::where('category','integral')->where('status',0)->where(['add_time'=>['<',$integral_withdrawal_time]])->select();
        if(count($integral_info)>0){
            foreach ($integral_info as $key => $value) {
                $user=UserModel::where('uid',$value['uid'])->find();
                if($user){
                    UserModel::bcInc($value['uid'],'integral',$value['number'],'uid');
                    UserBill::where(['id'=>$value['id']])->update(['balance'=>bcadd($user['integral'],$value['number'],2),'status'=>1,'qr_add_time'=>time()]);
                }
            }
        }
        //拼团成功
        $money_success_info=UserBill::where('category','now_money')->where('type','pin_success')->where('status',0)->where(['add_time'=>['<',$now_money_success_time]])->select();
        if(count($money_success_info)>0){
            foreach ($money_success_info as $key => $value) {
                $user=UserModel::where('uid',$value['uid'])->find();
                if($user){
                    UserModel::bcInc($value['uid'],'now_money',$value['number'],'uid');
        ...
        //拼团失败
        //拼团成功
        $money_fail_info=UserBill::where('category','now_money')->where('type','pin_fail')->where('status',0)->where(['add_time'=>['<',$now_money_success_time]])->select();
