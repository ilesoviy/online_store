...
<!--                        <button class="layui-btn layui-btn-sm layui-btn-normal" type="button" data-type="set_template"><i class="fa fa-check-circle-o"></i>发送模板消息</button>-->
                    ...
                    <script type="text/html" id="barDemo">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                            ...
                            <li>
                                <a href="javascript:void(0);" lay-event="delete_user">
                                    <i class="fa fa-eraser" aria-hidden="true"></i> 删除会员</a>
                            </li>
...
<script>
    ...
    layList.tableList('userList',"{:Url('get_user_list')}",function () {
        return [
                ...
                {field: 'integral', title: '佣金',width:'6%',sort:true,event:'integral'},
    ...
    layList.tool(function (event,data,obj) {
        ...
        switch (layEvent){
            ...
            case 'delete_user':
            $eb.$swal('delete',function(){
                    $eb.axios.get(layList.U({a:'delete_user',q:{uid:data.uid}})).then(function(res){
                        if(res.status == 200 && res.data.code == 200) {
                            $eb.$swal('success',res.data.msg);
                            obj.update({vip_name:false});
                            layList.reload();
                    ...
                    title:'您确定要清除【'+data.nickname+'】的会员吗？',
                    text:'清除后无法恢复请谨慎操作',
                    confirm:'是的我要清除'
                })
                break;
