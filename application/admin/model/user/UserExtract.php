...
class UserExtract extends ModelBasic
{
    ...
    public static function changeFail($id,$fail_msg)
    {
        $fail_time = time();
        $data =self::get($id);
        $extract_number=$data['extract_price']+$data['service_charge'];
        if($data['type_ti']==1){
            $mark='提現失敗，退回餘額 $'.$extract_number;
        $en_mark='Withdrawal failed, return balance $'.$extract_number;

        $title='提現失敗';
        $en_title='Withdrawal failed';
        }else{
             $mark='提現失敗，退回傭金'.$extract_number;
        $en_mark='Withdrawal failed, commission returned'.$extract_number;
        ...
        if($data['type_ti']==1){
            UserBill::income($title,$uid,'now_money','extract',$extract_number,$id,bcadd($User['now_money'],$extract_number,2),$mark,$en_title,$en_mark,1);
            User::bcInc($uid,'now_money',$extract_number,'uid');
        }else{
            UserBill::income($title,$uid,'integral','extract',$extract_number,$id,bcadd($User['integral'],$extract_number,2),$mark,$en_title,$en_mark,1);
            User::bcInc($uid,'integral',$extract_number,'uid');
        }
