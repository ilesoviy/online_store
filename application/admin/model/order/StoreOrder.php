...
//        $data=($data=$model->page((int)$where['page'],(int)$where['limit'])->select()) && count($data) ? $data->toArray() : [];
        foreach ($data as &$item){
            ...
            if($item['pink_id'] || $item['combination_id']){
                ...
                if($item['pin_type'] && $item['paid']==1){
                    switch ($item['pin_status']){
                    case 0:
                        $item['pink_name'] = '[拼团订单]未支付';
                        $item['color'] = '#895612';
                        break;
                    case 1:
                        $item['pink_name'] = '[拼团订单]正在进行中';
                        ...
                        break;
                    case 2:
                        $item['pink_name'] = '[拼团订单]拼团成功';
                        $item['color'] = '#00f';
                        break;
                    case 3:
                        $item['pink_name'] = '[拼团订单]拼团失败';
                        $item['color'] = '#f0f';
                        break;
                    default:
                        $item['pink_name'] = '[拼团订单]历史订单';
                ...
                }else if($item['pin_type'] && $item['paid']==0){
                    $item['pink_name'] = '[拼团订单]未支付';
                }else{
                    $item['pink_name'] = '[普通订单]';
                    $item['color'] = '#895612';
...
HTML;
            }else if($item['paid']==1 && $item['refund_status']==2 && $item['pin_type']==1&& $item['status']==0){
                $item['status_name']='已退款/未发货';
            }else if($item['paid']==1 && $item['refund_status']==2&& $item['pin_type']==1&& $item['status']==1){
                $item['status_name']='已退款/待收货';
            }else if($item['paid']==1 && $item['refund_status']==2&& $item['pin_type']==1&& $item['status']==2){
                $item['status_name']='已退款/已完成';
            }else if($item['paid']==1 && $item['refund_status']==2){
                $item['status_name']='已退款';
            }
            if($item['paid']==0 && $item['status']==0 && $item['refund_status']==0){
                $item['_status']=1;
            }else if(($item['paid']==1 && $item['status']==0 && $item['refund_status']==0)||($item['pin_type']==1&&$item['pin_status']==2&&$item['paid']==1 && $item['status']==0 && $item['refund_status']==2)){
                $item['_status']=2;
            }else if($item['paid']==1 && $item['refund_status']==1){
                $item['_status']=3;
            }else if($item['paid']==1 && $item['status']==1 && $item['refund_status']==0){
