...
class UserApi extends AuthController
{
    ...
    public function user_sign()
    {
        ...
        if($signed) return JsonService::fail(L('已簽到','Signed in'));
        if(false !== $integral = UserSign::sign($this->uid))
            return JsonService::successful(L('簽到獲得$','Check in to get$').floatval($integral),['integral'=>$integral]);
        else
            return JsonService::fail(UserSign::getErrorInfo(L('簽到失敗','Check in failed')));
    }
