...
// class Store extends IndexController
{

    public function index($keyword = '',$cid = '',$sid = '',$recommend=0)
    {
        ...
        $this->assign(compact('keyword','cid','sid','category','recommend'));
        return $this->fetch();
    }

    public function category($id = 18)
    {
        ...
        $_category = StoreCategory::all(function($query){
            ...
            $query->field('id,pid,'.$lang.'cate_name as new_cate_name,cate_name,pic')->where('is_show',1)->order('sort DESC');
        })->toArray();
        $category = [];
		...
		$id = $id;
		$cate = Db::name('store_category')->where('pid',$id)->select();
        $this->assign(compact('category','id','cate'));
		//var_dump($cate);die;
        return $this->fetch();
    }
    ...
    public function detail($id = 0)
    {
        ...
        $CollageInfo=Robot::getHead($id)->toArray();
        if(count($CollageInfo)==0){
            $CollageInfo=array_merge($CollageInfo,[[],[],[]]);
        }else{
           $CollageInfo=array_merge($CollageInfo,[[],[]]); 
        }
        //增加浏览量
        StoreProduct::where('id',$id)->update(['browse'=>$storeInfo['browse']+1]);
    ...
    public function confirm_order($cartId = '',$pin_type=0)
    {
        if(!is_string($cartId) || !$cartId ){
            return $this->failed('请提交购买的商品!');
        }
        $cartGroup = StoreCart::getUserProductCartList($this->userInfo['uid'],$cartId,1,$pin_type);
        // dump($pin_type);die;
