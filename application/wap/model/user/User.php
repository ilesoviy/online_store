...
class User extends ModelBasic
{
    ...
    public static function setRegisterUser($Userinfo,$spread_uid = 0)
    {  
        WechatUser::setWechatUser($Userinfo);
        if($spread_uid){
            WechatUser::yaoqingFirstSubGiveCoupon($spread_uid);
            $user_num=UserNum::where('user_id',$spread_uid)->whereTime('add_time', 'today')->find();
                 if($user_num){
                    UserNum::where('user_id',$spread_uid)->whereTime('add_time', 'today')->update(['user_num' => $user_num['user_num']+2]);
                    UserNum::income('增加',$spread_uid,2,$user_num['user_num']+2,'推荐用户增加两个购买次数');
                 }else{
                    $spread_uidcount=self::where('spread_uid',$spread_uid)->count();
                    $data=[
                        'user_id'=>$spread_uid,
                        'user_num'=>22+$spread_uidcount*2,
                        'add_time'=>time()
                    ];
                    UserNum::insert($data);
                    UserNum::income('初始化',$spread_uid,22+$spread_uidcount*2,22+$spread_uidcount*2,'推荐用户并初始化次数');
                 }
        }
        ...
        WechatUser::userFirstSubGiveCoupon($res);
        return $res;
    }
     ...
     * @return int $uid
     */
    public static function getActiveUids()
    {
        $uid = null;
        if(Session::has('loginUid','wap')) $uid = Session::get('loginUid','wap');
        return $uid;
