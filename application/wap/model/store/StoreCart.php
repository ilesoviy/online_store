...
class StoreCart extends ModelBasic
{
    ...
    public static function setCart($uid,$product_id,$cart_num = 1,$product_attr_unique = '',$type='product',$is_new = 0,$combination_id=0,$seckill_id = 0,$bargain_id = 0,$pin_type=0)
    {
        ...
        $where = ['type'=>$type,'uid'=>$uid,'product_id'=>$product_id,'product_attr_unique'=>$product_attr_unique,'is_new'=>$is_new,'is_pay'=>0,'is_del'=>0,'combination_id'=>$combination_id,'seckill_id'=>$seckill_id,'bargain_id'=>$bargain_id,'pin_type'=>$pin_type];
        if($cart = self::where($where)->find()){
            ...
            return self::set(compact('uid','product_id','cart_num','product_attr_unique','is_new','type','combination_id','seckill_id','bargain_id','pin_type'));
        }
    ...
    public static function getUserProductCartList($uid,$cartIds='',$status=0,$pin_type=0)
    {
        ...
        $productInfoField = 'id,image,slider_image,price,cost,ot_price,vip_price,pin_price,postage,mer_id,give_integral,cate_id,sales,stock,'.$lang.'store_name as store_name,en_store_name,unit_name,is_show,is_del,is_postage';
        $seckillInfoField = 'id,image,images as slider_image,price,cost,ot_price,pin_price,postage,give_integral,sales,stock,title as store_name,en_store_name,unit_name,is_show,is_del,is_postage';
        $bargainInfoField = 'id,image,min_price as price,price as ot_price,postage,pin_price,give_integral,sales,stock,title as store_name,en_store_name,unit_name,status as is_show,is_del,is_postage,cost';
        $model = new self();
        $valid = $invalid = [];
        if($cartIds)
...
//                    if ($cart['seckill_id'] != 0) {
                        if ($cart['product_attr_unique']) {
                            ...
                            if (!$attrInfo || !$attrInfo['stock']){
                                ...
                                $cart['truePrice'] = (float)$attrInfo['pin_price'];
                            ...
                            $cart['truePrice'] = (float)$cart['productInfo']['pin_price'];
