...
class StoreProduct extends ModelBasic
{
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public static function getHotSearch($field = '*',$limit = 0)
    {
        $model = self::where('is_del',0)->where('mer_id',0)
            ->where('stock','>',0)->where('is_show',1)->field($field)
            ->order('browse DESC,sort DESC, id DESC');
        if($limit) $model->limit($limit);
        return $model->select();
    }
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public static function threePink($field = '*',$limit = 0)
    {
        $model = self::where('spell_num',3)->where('is_del',0)->where('mer_id',0)
            ->where('stock','>',0)->where('is_show',1)->field($field)
            ->order('sort DESC, id DESC');
        if($limit) $model->limit($limit);
        return $model->select();
