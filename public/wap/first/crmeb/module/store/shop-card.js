(function (global,factory) {
    ...
    var template = '<div class=shop-card><div class=model-bg :class="{on:show == true}" @click=close @touchmove.prevent></div><div class=card-model :class="{up:show == true}"><div class=cm-header @touchmove.prevent><div class=header-product><img :src=product.image></div><div class=header-info><p class=money>$<i v-text=product.price></i></p><span class=stock><pan id="qkc">庫存</span><i v-text=product.stock></i></span></div></div><div id=selects-wrapper class=selects-info v-show="attrList && attrList.length > 0"><div class=scroll><div class=cm-selects v-for="(attr,index) in attrList"><h1>{{attr.attr_name}}</h1><div class="options option-color"><span v-for="value in attr.attr_values" :class="{on:attr.checked == value}" @click="changeChecked(value,attr,index)">{{value}}</span></div></div></div></div><div class="amount clearfix" @touchmove.prevent><div class="text fl">購買數量</div><div class="count fr"><div @click=descCartNum>-</div><input type=number v-model=cartNum readonly><div @click=incCartNum>+</div></div></div><div class=model-footer @touchmove.prevent><a class=footer-car href="javascript:void(0);" v-if="cart == true" v-show="product.stock > 0 && gocar" @click=goCart>加入購物車</a><a class=footer-buy href="javascript:void(0);" v-if="buy == true" v-show="product.stock > 0" @click=goBuy>立即購買</a><a class="footer-buy no" href="javascript:void(0);" v-show="!product.stock || product.stock <= 0">無貨</a></div><div class=model-close @click=close></div></div></div>';
    ...
    var shopCard = Vue.extend({
        ...
        methods:{
            ...
            _setData:function(opt){
                ...
                this.gocar = opt.gocar == undefined ? true : opt.gocar;
                this.init();
            }
        },
        mounted:function(){
